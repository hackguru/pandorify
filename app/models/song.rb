...
class Song < ActiveRecord::Base
  ...
  class << self
    ...
    def update_tiny_song_id
      ...
      list.each do |obj|
        # begin
          c = Curl::Easy.perform("http://tinysong.com/b/#{CGI.escape(obj.title.to_s.sub(" ","+") + obj.try(:album).name.to_s.sub(" ","+") + obj.try(:artist).name.to_s.sub(" ","+"))}?format=json&key=186bd60f3a33be26da02d62d334bddf4") # FROM Tinysong
