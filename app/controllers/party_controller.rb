class PartyController < ApplicationController
  ...
  def index
    ...
    begin    
      ...
      Requestedsong.find(:all, :conditions => ['party_id = ? AND added is not ?', @party.id, true]).each do |request|
      ...
      respond_to do |format|
         format.js
         format.json{
           render :json => {:matched => @songs.to_json, :requested => @party.songs.to_json}
         }
      end
      ...
      respond_to do |format|
         format.js
         format.json{
           render :json => {:matched => [].to_json, :requested => [].to_json}
         }
      end
    end      
